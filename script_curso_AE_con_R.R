...
modelo <- glm(cancer.p ~ diabetes_new + fumador_new, data=datos,family="binomial")
summary(modelo)
exp(cbind(OR=coef(modelo),confint(modelo)))
summary(modelo)$"coefficients"[2,1]

# Peticion de Carmen (OR de diabetes)
...
logit_test <- function(d,indices) {  
	...
	d <- d[indices,]  
	fit <- glm(cancer.p ~ diabetes_new + fumador_new, data=d,family="binomial")
...
final<-Sys.time()
tiempo<-difftime(final,inicio,units="mins")
tiempo
...
logit_test <- function(d,indices) {  
	...
	d <- d[indices,]  
	fit <- glm(cancer.m ~  as.factor(fumador) + bmi, data=d,family = "binomial") 
	return(coef(fit))  
...
explora_datos <- function(datos,grafico_numerico=FALSE,grafico_character=FALSE){
	...
	indice.numeric<-which(tipo%in%"numeric")
	indice.character<-which(tipo%in%"character")
	
	# Variables numericas
	
	res_numericas<-list()
	
	for(i in indice.numeric){
		
		
		
		tabla<-data.frame(variable=NA,media=NA,IC_media=NA,P25=NA,P50=NA,P75=NA,SD=NA,
			CV=NA,PNORM=NA)
		tabla$"variable"<-names(dk)[i]	
		tabla$"media"<-round(mean(dk[,i]),digits=2)
		tabla$"IC_media"<-paste(round(t.test(dk[,i])$"conf.int"[1],2),"-",
		round(t.test(dk[,i])$"conf.int"[2],2),sep="")
		tabla$"P25"<-round(quantile(dk[,i])[2],2)
		tabla$"P50"<-round(quantile(dk[,i])[3],2)
		tabla$"P75"<-round(quantile(dk[,i])[4],2)
		tabla$"SD"<-round(sd(dk[,i]),2)
		tabla$"CV"<-round(sd(dk[,i])/mean(dk[,i]),2)
		tabla$"PNORM"<-shapiro.test(dk[,i])$p.value
		res_numericas[[i]]<-tabla
	...
	library("plyr")
	tr_numeric<-ldply(res_numericas)
	
	if(grafico_numerico==TRUE){
		
		pdf("/Users/pfernandezn/Desktop/grafico_variables_numericas.pdf")
	
		
		par(mfrow=c(floor(length(indice.numeric)/2),floor(length(indice.numeric)/2)))
	
		for(i in indice.numeric){
			
			hist(dk[,i],main=names(dk)[i],xlab=names(names(dk)[i]))
	...
	res_character<-list()
	for(i in indice.character){
		
		tabla_check<-table(dk[,i],exclude=NULL)
		
		tabla<-data.frame(variable=NA,categorias=names(tabla_check),N=NA,POR=NA)
		
		tabla$"variable"[1]<-names(dk)[i]
		tabla$"N"<-as.numeric(tabla_check)
		tabla$"POR"<-prop.table(tabla_check)*100
		res_character[[i]]<-tabla
	...
	library("plyr")
	tr_character<-ldply(res_character)
	tr_character	
	
	if(grafico_character==TRUE)
	
		pdf("/Users/pfernandezn/Desktop/grafico_variables_categoricas.pdf")
	
		
		par(mfrow=c(floor(length(indice.character)/2),floor(length(indice.character)/2)))
	
		for(i in indice.character){
			
			barplot(table(dk[,i]),main=names(dk)[i])
	...
	list_of_datasets <- list("Variables numericas" = tr_numeric, "Variables caracter" = tr_character)
	unlink("/Users/pfernandezn/Desktop/descriptivos.xlsx")
	write.xlsx(list_of_datasets, file = "/Users/pfernandezn/Desktop/descriptivos.xlsx")

	
}


# Varias opciones de ejecucion

explora_datos(datos=datos)

explora_datos(datos=datos,grafico_numerico=TRUE)

explora_datos(datos=datos,grafico_numerico=TRUE,grafico_character=TRUE)
